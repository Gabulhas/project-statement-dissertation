...
Our experiment involves implementing a Proof of Work consensus protocol, similar to Bitcoin's. This serves as a learning exercise that allows us to understand the nuances of integrating a new consensus protocol into an existing blockchain like Tezos. We will discuss the requirements for this protocol, its integration process, and the tools we used for interaction.

Other components of the protocol, such as the client and the baker, will also be covered. The client serves as the gateway for network information, while the baker takes on the role of block creation and mining. We'll go into detail about the mining process, particularly the mechanism of finding the nonce that satisfies the block's hash conditions.

In summary, this chapter aims to demonstrate Tezos' capability to serve as a versatile platform for implementing and testing different consensus protocols. Our implementation of the Proof of Work protocol will not only serve as a proof of concept but also highlight Tezos' potential to support a broader range of consensus algorithms.
