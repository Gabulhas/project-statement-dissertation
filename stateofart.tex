...
Understanding these concepts is crucial for understanding how blockchains networks function in their core, the different trade-offs and design considerations involved in creating and operating a blockchain network. It starts with an overview of SMR and how it provides consistency and availability in decentralized systems.

Then, the concept of blockchain data structure and its application in decentralized networks is explored.
The different Network Models in distributed systems, such as synchronous, asynchronous, and partially-synchronous are also discussed. Finally, the section concludes with a exploration of the topic of consensus and it's importance in blockchain networks.
...
That is, every block, except for the first block (also called ``Genesis'' block), contains the hash of its previous (or parent) block.

Once a block is added to the blockchain, the block it's pointing to cannot be altered or deleted, making the ledger tamper-resistant and immutable and ensures data integrity, since, in order to change a block's information, one would need to change every child's block hash.
...
The blockchain can be considered the state of the machine, where multiple copies of the state of this machine are stored in multiple nodes, and newer states, that is, the process of appending a new block to the chain, are replicated to every machine in the network.
...
The decentralized nature of blockchain networks and the use of consensus mechanisms ensure that all nodes in the network have the same state of the chain, providing the availability to access of this information.

\subsection*{\textbf{Network Models}}

Network models are fundamental to the design and implementation of distributed systems. The three main types- synchronous, asynchronous, and partially-synchronous — each have unique properties that influence the behavior of algorithms and protocols. Understanding these differences is essential for creating effective and efficient systems.
\\
...
\begin{itemize}
    \item Finality or Agreement, that is, if (functioning) the participants must agree on a single specific value.
...
Figure \ref{fig:theorems} illustrates these theorems/properties, where it is possible to observe that the area where the 3 parts meet it is impossible to achieve.
...
In the following sections, more insight about these properties will be provided and why they are relevant to the topic of blockchain networks, such as the fact that networks like Bitcoin chooses Availability over Consistency (while being Partition Tolerant) and chooses Termination over Finality (while being fault-tolerant and having probabilistic finality).
...
Even though that in the context of most distributed systems the topic of decentralization isn't relevant, when talking about blockchain networks it is important, as it's one of the main reasons why people use these systems in the first place.

For example, Bitcoin is a highly decentralized and secure blockchain, as there are many nodes (more than 40 thousand  \cite{bitnodes}, as of the writing of this document), yet lacks scalability, since the creation of a block takes 10min and a transaction takes about 1 to 1.5 hours to complete/be confirmed \cite{gondek}\label{exampleoftime}.
...
In this section, we focus on key consensus algorithms that underpin blockchain networks. These algorithms are essential for network integrity and have evolved to address the ``Blockchain trilemma'' and unique network requirements. We'll explore their strengths, weaknesses, and notable implementations.
...
We will explain in detail this protocol since it was the first one to be used in a blockchain structure, because it is still one of the most relevant protocols used and we implemented it during the course of this project.
...
Regarding the FLP theorem, PoW exhibits Probabilistic Finality \cite{anceaume2020finality}. This means that all functioning nodes eventually agree on a single block, but the process is not deterministic and involves a certain degree of probability. The waiting for confirmations adds an additional layer of security to the consensus process. Like mentioned before in this example (\ref{exampleoftime}) in Bitcoin this takes about 6 confirmations, or about 1 hour, since a block usually takes 10 minutes to be created. This happens because mining is a process independent from the state of the network and independent from other nodes, and two or more nodes can solve the cryptographic puzzle/build a block ``at the same time'' (until the whole network agrees on the same block).
...
Once again, taking Bitcoin as an example, a node handles this case by using the Longest Chain Rule, where, when a node has two competing blocks, where each makes a branch, it waits until one of the branch is bigger than the other.

This is done because, the branch with the most computational power has a greater chance of being extended.
...
\begin{itemize}
    ...
    \item When a node receives a block from the network, it appends it to its own blockchain stored.
    ...
    \item The node tries out every possible value for the nonce until the resulting hash (SHA256 hash function) of the header has a value below the current target.
...
Besides block creation reward, the miner also receives these transactions fees, so they will select transactions based on that fee, and other attributes of the transactions.
...
Scalability remains a challenge in blockchain networks due to the need for security and decentralization. Various solutions aim to enhance transaction speed and network scalability: Decoupling of Blockchain Functions, Parallel Chains, and DAG-based Protocols.

\textbf{Decoupling of Blockchain Functions}: This approach separates block functions into key blocks for leader election and microblocks for transactions. It boosts network throughput but has latency issues and risks like leader compromise.

\textbf{Parallel Chains}: Miners extend multiple chains concurrently to increase throughput. However, this method is susceptible to targeted attacks on individual chains.

\textbf{DAG-based Protocols}: These protocols use a Directed Acyclic Graph (DAG) instead of a linear blockchain, allowing for concurrent block generation and a unique approach to scalability.
...
These miners join to mine a single block as if they were only a single node, so they have more computational power together, and inherently having a bigger chance of finding a block. When a block if found by the pool (a node, as viewed from the network), the reward is shared with all the miners, depending on how much effort they put into the mining.
...
It simplifies the complex task of implementing consensus algorithms, saving expertise and resources.
...
This project will provide feedback for the development of Lupin and may also serve as a layer of execution for the results of its compilation, and is in collaboration with the Lupin Language project and its results will be further exposed in a later section.
